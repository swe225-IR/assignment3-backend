...
from bckend.model.UniResponse import Result


class Indexer:
    def __init__(self, query: str, indexer_folder=r"D:\\Projects\\assignment3-search-engine\\data\\index\\indexer\\"):
    ...
    def get_page_rank(self):
        ...
        Calculate reversed rank
        '''
        with open('D:\\Projects\\assignment3-search-engine\\data\\index\\pgrk.pickle', 'rb') as f:
    ...
    def rank(self, tf_idf_weight=1.0, page_rank_weight=0.0):
        ...
        for value in page_rank:
            results.append(Result(str(i), value[1], {"url": value[0]}))
            i += 1
...
if __name__ == '__main__':
    ...
    query = sys.argv[1]
    indexer = Indexer(query=query)
    print(indexer.rank(tf_idf_weight=0.95, page_rank_weight=0.05))
    # indexer.rank()
